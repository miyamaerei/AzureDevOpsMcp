...
**目的**：建立本地项目与 Azure DevOps 项目的对应关系

**操作方式**：

```mcp
SetProjectMapping(localProjectName, azureDevOpsProjectId)
```
...
```mcp
GetAssignedTasks(userId, projectId)
```

**参数说明**：
- `userId`: 用户标识（可选，默认使用当前登录用户）
- `projectId`: 项目 ID（可选，默认使用默认映射项目）
