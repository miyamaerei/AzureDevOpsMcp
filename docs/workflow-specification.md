...
**目的**：建立本地 Git 仓库、GitHub 仓库与 Azure Boards 项目的对应关系。Azure DevOps Project 只是 Workitem 查询边界，当前任务归属边界是 GitHub 仓库。
...
**目的**：获取指派给当前用户，并且通过 GitHub 对象关联到当前仓库的任务列表。
...
**关联依据**：
- Azure Boards Workitem 链接的 `GitHub Branch`
- Azure Boards Workitem 链接的 `GitHub Commit`
- Azure Boards Workitem 链接的 `GitHub Pull Request`
- Azure Boards Workitem 链接的 `GitHub Issue`
- GitHub Commit / PR / Issue 描述中的 `AB#123` 反向关联
...
| 任务拉取为空 | 仓库映射未配置，或 Workitem 未链接当前 GitHub 仓库对象 | 配置仓库映射，并确认 GitHub Branch/Commit/PR/Issue 或 `AB#` 关联已建立 |
